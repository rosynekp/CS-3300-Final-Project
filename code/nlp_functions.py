...
"""
a dictonary expanding some common contractions greatly inspired by the following list: 
...
def remove_special_char(text, special_characters = ['~','@', '#', '$', '%', '^', '&', '*'], numeric = False):
    ...
    text: str
    numeric: boolean
      True if it is desired to not include digits
...
def remove_stopwords_punctuation(text, lang_model, lemmatizing=False, stop_words=False):
    ...
    if lemmatizing and stop_words:
        st= " ".join([token.lemma_ for token in doc_text if not(token.is_punct) and not(token.is_stop)]) 
    elif lemmatizing:
        st= " ".join([token.lemma_ for token in doc_text if not(token.is_punct)])
    elif stop_words:
        st= " ".join([token.text for token in doc_text if not(token.is_punct) and not(token.is_stop)])
    else:
...
def normalize_text(text, lang_model, special_characters = ['~','@', '#', '$', '%', '^', '&', '*'], numeric = False, lemmatizing=False, stop_words=False):
    """
    This function normalizes or pre-processes the text.
    ...
    lang_model: spacy Language object
    lemmatizing: boolean
        True if it is desired to lemmatize the words
    stop_words: boolean
        True if it is desired to remove stop_words
    ...
    filtered_text =  remove_stopwords_punctuation(text, lang_model, lemmatizing, stop_words)
